...
class Actors extends Controller
{
    public function process() { 
        ...
        if($Filter['_ACTION'] == 'filter') {  
            ...
            foreach ($Filter as $key => $value) {
                if(in_array($key, $FilterPermissions)) {
                    $Where .= ' actors.'.$key.' = "'.$value.'" AND ';
                }
            }
            $Where = $WhereC.' '.rtrim($Where,' AND ');  
        } elseif($Filter['search']) {
            $Where .= 'WHERE ';
            $Where .= 'actors.name LIKE "%'.$Filter['search'].'%" AND'; 
            $Where = rtrim($Where,' AND ');  
        }
        $Where = rtrim($Where,' AND ');  
        if($Filter['sortable']) { 
            $OrderBy = 'ORDER BY actors.id '.$Filter['sortable'];
        }else{
            $OrderBy = 'ORDER BY actors.name ASC';
        }      
        // Query 
    	...
    	if(isset($AuthUser['account_type']) == 'admin') {
        	...
        	if($Submit['id'] AND $Submit['_ACTION'] == 'delete') {
            	$this->db->delete('actors')->where('id',$Submit['id'],'=')->done(); 
            	$Notify['type']     = 'success';
