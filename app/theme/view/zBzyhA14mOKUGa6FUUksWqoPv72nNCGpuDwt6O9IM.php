...
$newpremium = date('Y-m-d', strtotime('+6 month', strtotime($today)));;
$plusmonth = date('Y-m-d', strtotime('+6 month', $premiumexpire));
 ...
 if (strtotime($today) > strtotime($premiumexpire)) {
	...
	$sql = "UPDATE users SET date = DATE_ADD(date, INTERVAL 6 month ) WHERE username='$username'";
    $sql1 = "UPDATE users SET account_type = 'premium' WHERE username='$username'";
}
