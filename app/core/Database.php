...
class Database extends \PDO
{
    ...
    public function showPagination($url=null, $class = 'active',$small=null) {
        if ($this->totalRecord > PAGE_LIMIT) {
            ...
            if($_GET['page']!=1 AND $_GET['page']) {
                $this->html .= '<li class="page-item"><a class="page-link border-0" href=\''.str_replace('[page]',$this->prevPage(), $url).'\' rel="nofollow">'.__('Prev').'</a></li>';
            }
            ...
            if($_GET['page']!=$this->nextPage()) {
                $this->html .= '<li class="page-item"><a class="page-link border-0" href=\''.str_replace('[page]',$this->nextPage(), $url).'\' rel="nofollow">'.__('Next').'</a></li>';
            }
            $this->html .= '</ul>';
            return $this->html;
        }
